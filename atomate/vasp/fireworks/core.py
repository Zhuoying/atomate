...
class ConvergeFW(Firework):
    ...
    def __init__(self, structure=None, name="static", vasp_input_set=None, vasp_input_set_params=None,
                 vasp_cmd="vasp", prev_calc_loc=True, prev_calc_dir=None, db_file=None, parents=None, **kwargs):
        """
        Standard convergence calculation Firework - either from a previous location or from a structure.

        Converges k-point mesh and energy cutoff.
        ...
        t.append(RunVaspCustodian(vasp_cmd=vasp_cmd, job_type='converge',
                                  auto_npar=">>auto_npar<<"))
        t.append(PassCalcLocs(name=name))
        t.append(
            VaspToDb(db_file=db_file, additional_fields={"task_label": name}))
